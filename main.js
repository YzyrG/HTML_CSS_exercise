...
// 以上的个性化设置不完整，当用户刷新页面或关闭页面后再进入，标题会被还原，刚刚设置的名字不见了
//改进，存储名字
let myHeading = document.querySelector('h1');
let myButton = document.querySelector('button');
//程序是按照代码顺序执行的，要让页面刷新后标题不会被还原，则在用户进入页面点击按钮前，去判断是否已存在名字
// 存在，则调用保存的名字显示对应标题；不存在则由用户自己点击按钮创建
// 所以，判断逻辑要放在按钮点击事件之前，而判断，需要要调用已经创建的名字，因此创建函数要在判断逻辑之前
// 三部分逻辑的顺序是：创建函数>判断逻辑>按钮点击事件
